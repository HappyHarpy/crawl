...
bool melee_attack::handle_phase_hit()
{
    ...
    // for SP_WIGHT: without damage, only drain.
        ...
        && you.has_mutation(MUT_NEGATIVE_ENERGY_ATTACK))
    {
        if (defender->alive() && defender->holiness() & MH_NATURAL)
        {
            ...
            if (defender->drain_exp(attacker, true, 10 + power))
            {
                if (defender->is_player())
                    obvious_effect = true;
            }
    ...
    if (attacker->is_cigotuvis_host() && defender->alive())
    {
        int dur = 10;
        if (defender->is_monster())
            dur = 10 * BASELINE_DELAY;
        defender->cigotuvis_infection(dur, false);
    }
...
bool melee_attack::consider_decapitation(int dam, int damage_type)
{
    ...
    const int limit = defender->type == MONS_LERNAEAN_HYDRA             ?     27:
                      (defender->is_player() && you.has_hydra_multi_attack())?100 - you.get_mutation_level(MUT_MISSING_NECK):
                                                                              MAX_HYDRA_HEADS;
